<script setup>
// Importy i konfiguracja komponentu
import SectionTitle from "@/components/ui/SectionTitle.vue";
import { ref, onMounted, onUnmounted } from "vue";
import { getStreakLeaderboard } from "@/components/services/userProgress.js";

// Import domeny
import { pluralizeDay } from "@/domain/text";

// Lista danych do rankingu
const rows = ref([]);
...
async function reload() {
  loading.value = true;
  rows.value = await getStreakLeaderboard(5);
  loading.value = false;
}

// Po zamontowaniu komponentu pobieramy ranking streak
onMounted(async () => {
  try {
    rows.value = await getStreakLeaderboard(5);
  } catch (error) {
  ...
  // Odświeżamy ranking przy zmianie preferencji
...
<template>
  <!-- Sekcja streak rankingu -->
  <section
    id="streak-ranking"
    class="py-14 bg-section scroll-mt-28 md:scroll-mt-32"
  >
    <div class="max-w-6xl mx-auto px-4">
      <!-- Tytuł sekcji -->
      <div class="animate-fade-slide-delay-5">
        <SectionTitle
          eyebrow="Seria dni"
          title="Najbardziej wytrwali"
          subtitle="Kto utrzymał najdłuższy streak codziennej nauki?"
        />
      </div>
      ...
      <div
        class="hidden sm:block overflow-hidden rounded-3xl border border-border bg-card shadow-sm hover:shadow-md transition mt-6 animate-fade-slide-delay-6"
      >
        <!-- Tabela wyników -->
        <table class="w-full text-sm tabular-nums">
          ...
          <thead class="bg-card">
            <tr class="text-primary">
              ...
              <th class="px-4 py-4 text-right font-medium">Dni z rzędu</th>
            ...
            <template v-if="loading">
              <tr v-for="n in 5" :key="n" class="animate-pulse">
                ...
                <td class="px-4 py-4 text-right">
                  <div class="h-4 w-14 ml-auto rounded skeleton"></div>
                </td>
                <td class="px-4 py-4 text-right">
                  <div class="h-4 w-10 ml-auto rounded skeleton"></div>
                </td>
              </tr>
            </template>
